/-
  ...
  Theorem 2(iii) Step 3: The PSD-constrained MLE is closer to the unconstrained
  optimum than the true state is, on measured coordinates.
  ...
  Derived (machine-verified, taking the relevant bounds as direct hypotheses):
    - psd_contraction: Σ(y_P(ρ̂) - ŷ_P)² ≤ C·Σ(x_P - ŷ_P)²  [C = 1/(1-t★²) ≥ 1]
...
/-! ## PSD contraction (Proposition 4 in SM)
  ...
  From the two axioms (with contraction prefactor C = 1/(1−t★²) ≥ 1):
...
/-- PSD contraction with exact prefactor C ≥ 1.
    ...
    The MLE output satisfies:
      Σ(y_P(ρ̂) - ŷ_P)² ≤ C · Σ(x_P - ŷ_P)²
    where C = 1/(1−t★²) arises from the lower Taylor curvature bound.
...
/-! ## Factor (2+2C) triangle inequality bound
  ...
  Since C = 1/(1−t★²) = 1 + O(ln(M/δ)/N_s), the prefactor (2+2C)
...
/-! ## Tightness of factor (2+2C)
  ...
  The exact prefactor is 2+2C where C = 1/(1−t★²) ≥ 1.
...
/-! ## Unconditional finite-sample chain (Fix-B, 2026-04-07; Fix-C, 2026-04-13)
  ...
  coefficient C·N_s/2 (where C = contractionPrefactor = 1/(1−t★²)),
...
/-- **Unconditional PSD contraction** (Theorem 6(iii) Step 3, formal version).
    ...
    `(M, N_s)` with `0 < N_s`, the PSD-constrained MLE satisfies:
        Σ(y_P(ρ̂) − ŷ_P)² ≤ C · Σ(x_P − ŷ_P)²
    where C = contractionPrefactor M N_s = 1/(1−t★²) ≥ 1. -/
...
/-- **Unconditional factor-4 bound** (leading-order version).

    Since C ≥ 1, the (2+2C) bound implies (2+2·1) = 4 as a coarser bound.
    This is the version stated in the manuscript (leading-order). -/
theorem factor_four_unconditional
    {d : ℕ} (ρ : DensityMatrix d) (M N_s : ℕ) (hN : 0 < N_s) :
    totalErrSq ρ M N_s ≤ 4 * blockDevSq ρ M N_s := by
  have h_exact := factor_2plus2C_unconditional ρ M N_s hN
  have hC := contractionPrefactor_ge_one M N_s
  nlinarith
...
/-- **Leading-order combined shot-noise bound** (factor 4 version).
    Coarser but matches the manuscript's displayed equation. -/
theorem combined_shot_noise_unconditional_leading
    {d : ℕ} (hd : 0 < d) {dq : ℕ} (ρ : DensityMatrix dq)
    (M N_s : ℕ) (δ : ℚ) (hN : 0 < N_s)
    (h_event : HoeffdingEvent ρ M N_s δ) :
    totalErrSq ρ M N_s / d ≤ 4 * hoeffdingDevBound M N_s δ / d := by
  have hd_q : (0 : ℚ) < (d : ℚ) := by exact_mod_cast hd
  have h_factor4 := factor_four_unconditional ρ M N_s hN
  have h_hoeff := hoeffding_block_dev_bound ρ M N_s δ h_event
  have h_combined : totalErrSq ρ M N_s ≤ 4 * hoeffdingDevBound M N_s δ := by
    linarith
  exact div_le_div_of_nonneg_right h_combined (le_of_lt hd_q)

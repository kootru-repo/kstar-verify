...
/-! ### Contraction prefactor C = ((1+t★)/(1−t★))² ≥ 1
    The per-operator log-likelihood ℓ_P(y) has second derivative
    ℓ_P″(y) = −n₊/(1+y)² − n₋/(1−y)², so for |y| ≤ t★ < 1:
        N_s/(1+t★)² ≤ |ℓ_P″(y)| ≤ N_s/(1−t★)².
    The ratio of upper to lower Hessian bounds gives the contraction
    prefactor C = (1+t★)²/(1−t★)² = ((1+t★)/(1−t★))².
    Under the Hoeffding event, t★ = O(√(ln(M/δ)/N_s)), so
    C = 1 + O(t★); the manuscript uses the leading-order
    value C = 1 (and hence factor 4 rather than 2+2C). -/

/-- Contraction prefactor: ((1+t★)/(1−t★))², where t★ is the
    Bloch-coordinate bound from the Hoeffding event. Declared opaque
    (like blockDevSq) so no concrete rational can be substituted. -/
axiom contractionPrefactor : ℕ → ℕ → ℚ

/-- C ≥ 1, since ((1+t★)/(1−t★))² ≥ 1 for 0 ≤ t★ < 1. -/
...
/-! ### Axiom 8 (Hradil 1997 -- lower Taylor for the true state)
    ...
    true state ρ. Uses the curvature bound |ℓ_P″| ≤ N_s/(1−t★)²,
    encoded via `contractionPrefactor`:
        L(ρ) ≥ L_unc − (C · N_s / 2) · Σ(x_P − ŷ_P)²
    where C = contractionPrefactor M N_s = ((1+t★)/(1−t★))².
